...
if (!Ext) {
    ...
    Ext.override(Ext.Toolbar, {
        insert: function (index, el) {
            ...
            if (index >= this.tr.childNodes.length) {
...
AnnoJ.Bookmarker = (function () {
    ...
    function render(bookmark) {
        ...
        nam.on('click', function (event) {
            event.stopEvent();
            console.log(row.bookmark.location);
    ...
    function remove(bookmark) {
        var clean = [];
        Ext.each(bookmarks, function (item) {
            if (item != bookmarks) {
                clean.push(item)
            }
        });
...
//alert(img);
		  if (!img) return;
//		  
//img.src = img.src.replace("http://bioinformatics.psb.ugent.be/webtools/sandbox/liste/BOGAS/trunk/www/site/annoj/Ectsi/","");
//img.src = img.src.replace("%22","");
//http://bioinformatics.psb.ugent.be/webtools/sandbox/liste/BOGAS/trunk/www/site/annoj/Ectsi/
//alert (img.src);
//alert (img.id);
		  if (!img.complete ) {
